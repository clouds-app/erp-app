<template>
	<view>
		<cu-custom bgColor="bg-gradual-blue" :isBack="true">
				<block slot="content">纸箱订单汇总表</block>
		</cu-custom>
			...
			<view class="flex border-top">
				<view class="flex-sub">
					<selectDropdown
						...
						:params="{modelCode:'box_order_sum'}"
					></selectDropdown>
				...
				<view class="flex-sub">
					<view class="cu-form-group">
						<view class="fixed-left">
							...
							<button :disabled="isLoaddingData" :style="{color: formItem.searchMode=='3'?'red':''}" class=" margin-left-custom cu-btn round line-green" @click="searchModeClick('3')" >盒式</button>
...
export default {
	...
	methods: {
		...
		loaddingData(){
			...
			this.sumMoney =0
			let url = '/warehouse/warehouse/execute/spBoxCOAnalyzer'
			let params = {
		...
		formatDataListBy(dataList){
			if(dataList && Array.isArray(dataList) && dataList.length>0)
			{
				dataList = dataList.map(item=>{
					...
					// if(!!item.co_RateM){
					// 	item.co_RateM = Number(item.co_RateM).toFixed(1)
					// }
