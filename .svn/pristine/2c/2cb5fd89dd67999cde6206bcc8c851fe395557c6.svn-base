<template>
	<view>
		<cu-custom bgColor="bg-gradual-blue" :isBack="true"><block slot="content">纸箱半成品入库</block></cu-custom>
...
export default {
	...
	methods: {
		...
		aspSaveBoxOutTemp() {
			...
			this.getWarehouseDataProvider('aspSaveBoxOutTemp', data).then(res => {
				if (res.list.length > 0) {
					if (res.list[0].ErrorStr && res.list[0].ErrorStr != '') {
						this.toast.message(res.list[0].ErrorStr);
						return;
			...
			this.getWarehouseDataProvider('boxScanOrder', data)
				.then(res => {
					if (res.list && res.list.length > 0) {
						...
						this.toast.message('没有该工单对应的数据');
						this.boxInc.formItem.hp_bi_WorkNo = ''
					}
				})
				.catch(err => {
					//this.errorContent = '暂无数据';
		...
		aspSaveBoxHalfProdIn() {
			...
			this.getWarehouseDataProvider('aspSaveBoxHalfProdIn', data).then(res => {
				if (res.list.length > 0) {
					if (res.list[0].ErrorStr && res.list[0].ErrorStr != '') {
						this.toast.message(res.list[0].ErrorStr);
						return;
