<template>
	<view>
		<cu-custom bgColor="bg-gradual-blue" :isBack="true">
				<block slot="content">纸板订单查询</block>
		</cu-custom>
		...
		<view class="border-bottom grid-warp" v-for="(item,index) in orderList">
			<view class="grid-title">
				...
				<!--  -->
				<view  :class="[item.statue=='N'?'bg-gray':'bg-red','radius','cu-tag']">{{item.statue=='N'?'未录入':'已录入'}}</view>
			</view>
			<view class="grid-body">
				<view class="grid-flex padding-10">
					...
					<view>
						<text>下单:{{item.saleName}}</text>
					</view>
				</view>
				<view class="grid-flex padding-10">
					<view>
						<text>纸质:{{item.customerArt}}</text>
					</view>
					<view>
						<text>规格(长*宽):{{item.paperLength+'*'+item.paperWidth}}</text>
					</view>
				</view>
				<view class="grid-flex padding-10">
					<view>
					...
					<view>
						<text>压线:{{item.yxStr}}</text>
					</view>
				</view>
				<view class="grid-flex padding-10">
					<view>
						<view v-show="item.statue=='N'&&item.saleId===userInfo.erpUserCode?true:false" @click.stop="butclick(0,index)" class=" cu-tag round light line-gray">取消订单</view>
					</view>
					<view>
						<view  @click.stop="butclick(1,index)" class=" cu-tag round light line-green">订单详情</view>
					</view>
					<!-- <view>
						<view v-show="item.statue=='N'?true:false"  @click.stop="butclick(1,index)" class=" cu-tag round light line-grey">修改订单</view>
					</view> -->
					<!-- <view>
						<view @click.stop="butclick(2,index)" class=" cu-tag round light line-green">再次下单</view>
					</view> -->
				</view>
			</view>
		</view>
					...
					<view class="flex border-top">
						<view class="flex-sub">
							<view class="cu-form-group">
								...
								<input  v-model="hideModalList.customerArt"/>
							</view>
						</view>
						<view class="flex-sub">
							<view class="cu-form-group">
								<view class="title">楞ㅤ别:</view>
								<input  v-model="hideModalList.artLB"/>
							</view>
						</view>
					</view>
					<view class="flex border-top">
						<view class="flex-sub">
							<view class="cu-form-group">
								<view class="title">纸ㅤ长:</view>
								<input type="number" v-model="hideModalList.paperLength"/>
							</view>
						</view>
						<view class="flex-sub">
							<view class="cu-form-group">
								<view class="title">纸ㅤ宽:</view>
								<input type="number" v-model="hideModalList.paperWidth"/>
							</view>
						</view>
					</view>
					<view class="flex border-top">
						<view class="flex-sub">
							<view class="cu-form-group">
								...
								<input type="number" v-model="hideModalList.boxHeight"/>
							</view>
						</view>
						<view class="flex-sub">
							<view class="cu-form-group">
								<view class="title">数ㅤ量:</view>
								<input type="number" v-model="hideModalList.qty"/>
							</view>
						</view>
					</view>
					<view class="flex border-top">
						<view class="cu-form-group">
							...
							<input  v-model="hideModalList.lineType"/>
						</view>
					</view>
					<view class="flex border-top">
						<view class="cu-form-group">
							<view class="title">交货日期:</view>
							<input v-model="hideModalList.dueDate"/>
						</view>
					</view>
					<view class="flex border-top">
						<view class="cu-form-group">
							<view class="title">压ㅤㅤ线:</view>
							<input  v-model="hideModalList.yxStr"/>
						</view>
					</view>
					<view class="flex border-top">
						<view class="cu-form-group">
							<view class="title">送货备注:</view>
							<input  v-model="hideModalList.remark"/>
						</view>
					</view>
					<view class="flex border-top">
						<view class="cu-form-group">
							<view class="title">生产备注:</view>
							<input  v-model="hideModalList.produceRemark"/>
						</view>
					</view>
				</view>
			</view>
...
import dayjs from 'dayjs'
	export default {
		...
		methods: {
			...
			requestData(data){
				return request.post('/warehouse/warehouse/execute/appGetPaperOrderList',data)
			},
			// 分页查询
			getmoreorderList(){
			...
			modifyOrder(type){
				...
				// navigateTo  保留此页面
				uni.redirectTo({
				    url: `../placeanOrder/placeanOrder_cqzh?id=${this.buttonindex}&type=${type}`
				});
			},
			// 取消订单确认事件
			confirmFrom(){
				...
				this.toast.loading()
				request.post('/warehouse/warehouse/execute/appPaperOrderDelete',{id:data}).then(res=>{
					this.getorderList()
