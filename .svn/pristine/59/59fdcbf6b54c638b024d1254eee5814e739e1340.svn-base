		...
		<view class="border-bottom grid-warp" v-for="(item,index) in orderList">
			...
			<view class="grid-body">
				<view class="grid-flex padding-10">
					...
					<view>
						<text>楞别:{{item.artLB}}</text>
					</view>
				</view>
				<view class="grid-flex padding-10">
					<view>
						<text>纸长:{{item.paperLength}}</text>
					</view>
					<view>
						<text>纸宽:{{item.paperWidth}}</text>
					</view>
				</view>
				<view class="grid-flex padding-10">
					<view>
...
import dayjs from 'dayjs'
	export default {
		...
		methods: {
			...
			requestData(data){
				return request.post('/warehouse/warehouse/execute/appGetPaperOrderListCzln',data)
			},
			// 分页查询
			getmoreorderList(){
			...
			modifyOrder(type){
				...
				// navigateTo  保留此页面
				uni.navigateTo({
				    url: `../placeanOrder/placeanOrder_czln?id=${this.buttonindex}&type=${type}`
				});
			},
			// 取消订单确认事件
			confirmFrom(){
