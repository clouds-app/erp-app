<template>
	<view>
		<approval listLoadUrl="/approval/boxSpecPrice/approvalList" ref="approval">
				...
				<view class="grid-warp" @tap="openDetail(data,index)">
					...
					<view class="grid-body">
						...
						<view class="grid-flex padding-10">
							...
							<view>
								<text></text>
								<text class="text-price">{{data.bisPrice}}</text>
							</view>
						</view>
					</view>
				</view>
...
export default {
	...
	methods: {
		openDetail(data,index){
			...
			this.router.navigate(`/pages/approval/boxSpecPrice/detail?id=${data.ID1}`)
		}
	},
	onReachBottom(res) {
