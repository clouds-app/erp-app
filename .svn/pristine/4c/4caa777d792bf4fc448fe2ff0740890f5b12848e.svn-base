		...
		<form class="xunhoutop">
			<view class="flex border-top">
				<view class="flex-sub">
					...
					<button :disabled="isLoaddingData"  class="cu-btn  line-blue" @click="searchDataBy('week')" >本周</button>
					<button :disabled="isLoaddingData"  class="cu-btn  line-blue" @click="searchDataBy('month')" >本月</button>
			...
			<view class="flex border-top" style="margin-bottom: -3%;">
				<view class="flex-sub">
					<view class="cu-form-group" style="height: 75%;">
						<view class="title">状态</view>
						<picker @change="PickerChange" :value="formItem.index" :range="picker">
							<view class="picker">
								{{formItem.index>-1?picker[formItem.index]:'请选择状态'}}
							</view>
						</picker>
					</view>
				</view>
				<view class="flex-sub">
					<selectDropdown
						...
						:params='{modelCode:"paper_order_status",pageNum:1,pageSize:100,UserCode:userInfo.erpUserCode}'
					></selectDropdown>
				</view>
			</view>
		...
		methods: {
			...
			getorderList(){
				let data = {
					...
					customerId:this.formItem.CustID
				}
				this.toast.loading()
