<template>
	<view>
		...
		<view class="flex border-top">
			...
			<view class="flex-sub">
				<view class="cu-form-group">
					...
					<input  @blur="closeMain" class="borderBtm" v-model="formItems.customerArt"/>
				</view>
			</view>
			<view class="flex-sub">
				<selectDropdown
					...
					:params='{modelCode:"paper_price_calculator",pageSize:100,pageNum:1,ArtId:formItems.customerArt}'
				></selectDropdown>
			</view>
		</view>
		<!-- <view class="flex border-top">
		...
		<view class="bottomText">
			<view style="color: red;margin-top: 40rpx; font-size: 30rpx;">
				说明:原纸成本,取财务管理原纸进价设定中明细执行价
			</view>
			<view style="color: red;margin-bottom: 120rpx; font-size: 30rpx;">
				输纸质时,如配纸有多位,请以空格分隔.样例:H1 5 3 5 a
			</view>
	...
	export default {
		...
		data() {
			return {
				...
				pzList:Object.assign({},default_pzList),//配纸
			}
		},
		// watch:{
		...
		methods: {
			...
			//选择纸质回调事件
			closeMain(data){
				if(!!!this.formItems.customerArt) return
				let val = this.formItems.customerArt.toUpperCase()
				if(val.indexOf(' ')==-1){
					if(val.length > 9){
						this.toast.message('多个纸质请用逗号分割')
						return
					}else{
						this.setChangPaper(val.split(''))
					}
				}else{
					this.setChangPaper(val.split(' '))
				}
				...
				// this.getmatchingPaper(this.formItems.customerArt)
			},
			// 纸质改变渲染配纸
			setChangPaper(data){
				if(data.length%2==0){
				...
				let dataList = []
				data.map((item,index)=>{
					dataList.push({ArtCS:index+1,ArtPZ:item})
				})
				this.$refs.matchingPaper.ChangePaper(dataList)
				this.ChangePaper(dataList)
			},
			// 选择纸质查询配纸
			// getmatchingPaper(data){
			// 	this.toast.loading()
			// 	let _self = this
			// 	request.post('/warehouse/warehouse/execute/appGetArtPzByArtId',{ArtId:data.toUpperCase()}).then(res=>{
			// 		_self.$refs.matchingPaper.ChangePaper(res.list)
			// 		_self.ChangePaper(res.list)
			// 		_self.toast.hide()
			// 	}).catch(err=>{
			...
			ChangePaper(data){
				...
				data.filter(item=>{
					...
					if(!!item.ArtPZ){
						_self.pzList[Fromfield] = item.ArtPZ
					}
				})
				this.getpaperCost()
			},
			...
			getpaperCost(data){
				...
				let patams = {
					...
					ArtId:this.formItems.customerArt.toUpperCase(),
