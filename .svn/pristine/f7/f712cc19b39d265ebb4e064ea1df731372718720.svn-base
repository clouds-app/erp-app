			...
			<view slot="content" slot-scope="{data}">
				<view class="grid-warp">
					<view class="grid-title">
						{{initData.pr_Depart==null? ' - ':initData.pr_Depart+'-'+initData.pr_Man}}
					</view>
					<view class="grid-body">
						<view class="grid-flex padding-10">
							<view>
								<text>单号:{{ initData.pr_Code }}</text>
							</view>
							<view>
								<text>日期:{{ initData.pr_Date }}</text>
							</view>
						</view>
						<view class="grid-flex padding-10">
							<view>
								<text>收款单位:{{ initData.pr_PayeeName }}</text>
							</view>
						</view>
						<view class="grid-flex padding-10">
							<view>
								<text>货币:{{ initData.pr_CuryId }}</text>
							</view>
							<view>
								<text>金额:{{ initData.pr_Amt }}</text>
							</view>
						</view>
						<view class="grid-flex padding-10">
							<view>
								<text>付款项目:{{ initData.pr_PayProject	}}</text>
							</view>
						</view>
						<view class="grid-flex padding-10">
							<view>
								<text>付款单据号:{{ initData.pr_PayBill }}</text>
							</view>
						</view>
					</view>
					<!-- 审核详情 -->
					...
					<examine-Modal 
						...
						@hideModal='hideModal' 
						modelName="paymentRequest/level1">
					</examine-Modal>
	...
	export default {
		...
		methods: {
			approvalConfirm(type){
				...
				this.paperSpecPricedata.spNo = this.initData.pr_Code
			...
			getAuditdetails() {
				let data = {
					recordNo: this.initData.pr_Code,
					modelCode:'approval_paymentRequest1'
				};
				this.toast.loading();
				request
