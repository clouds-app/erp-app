<template>
	<view>
		<cu-custom bgColor="bg-gradual-blue" :isBack="true">
				<block slot="content">业务销售业绩</block>
		</cu-custom>
			...
			<view class="flex border-top">
				<view class="flex-sub">
					<selectDropdown
						...
						:params="{modelCode:'salerAnalyzer_sum'}"
					></selectDropdown>
				...
				<view class="flex-sub">
					<view class="cu-form-group">
						<view class="fixed-left">
							...
							<button :disabled="isLoaddingData" :style="{color: formItem.searchMode=='1'?'red':''}" class=" margin-left-custom cu-btn round line-brown" @click="searchModeClick('1')" >日期</button>
...
export default {
	...
	data() {
		return {
			...
			dataColumnsCopy:[
				...
				{ key: 'co_Money', title: '订单金额', $width: 230, titleAlign: 'center', columnAlign: 'right' },
				{ key: 'pd_Money', title: '送货金额', $width: 230, titleAlign: 'center', columnAlign: 'right' },
				{ key: 'RemainCredit', title: '可用信用', $width: 220, titleAlign: 'center', columnAlign: 'right' },
				{ key: 'ct_LastOrderdate', title: '最后下单', $width: 180, titleAlign: 'center', columnAlign: 'right' }
			]
		};
	},
	computed:{
	...
	methods: {
		...
		searchModeClick(type,loadDataNow=true){
			...
			switch (type){
				case '0':
				   item = { key: 'ct_Desc', title: '客户', $width: 250, titleAlign: 'center', columnAlign: 'center' }
					break;
				case '1':
				this.dataColumns.splice(-1,1)
				this.dataColumns.splice(-1,1)
				 item = { key: 'days', title: '日期', $width: 250, titleAlign: 'center', columnAlign: 'center' }
						break;
		...
		loaddingData(){
			...
			this.sumMoney =0
			let url = '/warehouse/warehouse/execute/aspSalerAnalyzerAPP'
			let params = {
				sFrom:this.formItem.startDate, //--起始日期
				sTo:this.formItem.endDate,       //  --结束日期   
				mode:this.formItem.searchMode,     //  --0:客户，1：日期
				cList:this.formItem.guestId,       //    --客户
				//ct_SalerId:this.formItem.businessId,//      --业务员ID      
		...
		formatDataListBy(dataList){
			if(dataList && Array.isArray(dataList) && dataList.length>0)
			{
				dataList = dataList.map(item=>{
					if(!!item.co_Money){
						item.co_Money = Number((item.co_Money).toFixed(2)).toLocaleString()
					}
					if(!!item.pd_Money){
						item.pd_Money = Number((item.pd_Money).toFixed(2)).toLocaleString()
					}
					if(!!item.RemainCredit){
						item.RemainCredit = Number((item.RemainCredit).toFixed(2)).toLocaleString()
					}
					return item
				})
			}
