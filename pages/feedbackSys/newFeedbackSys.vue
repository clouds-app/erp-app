...
<!-- 		<view class="flex bg-white padding-bottom-xs">
		...
		<view class="flex bg-white padding-top-xs solid-top padding-bottom-xs">
			...
			<view class="flex-sub">
				<view class="flex text-sm align-center justify-start base-height padding-right-xs">(回复时间: 工作日 8:00-20:00 非工作日 9:00-17:00)</view>
			</view>
		</view>
		<view class="flex bg-white">
			<view class="flex-sub padding-10">
				<textarea v-model="formDataItem.content" maxlength="200"  placeholder="请输入问题详情 (最多200汉字)" style="width: auto; height: 230rpx;border: 1px solid #ccc;" ></textarea>
			</view>
		</view>
		<view class="flex bg-white">
			<view class="flex-sub">
				<view class="flex-title base-height justify-start align-center">
					<text class="margin-left-sm">图片上传 (最多3M/4张)</text>
				</view>
			</view>
...
<!-- 		<view class="flex bg-white">
			<view class="flex-width-15 margin-left-sm">
			...
			<view class="flex-sub flex ">
				<view class="flex-width-50 base-height">
					<view class="flex">
						<view class="flex-sub base-height flex align-center">
							<input style="font-size: 26rpx;" :disabled="true" @tap="handleTap('provinceName')" v-model="provinceName" placeholder="选择所属省" name="provinceName"/> 
						</view>
				...
				<view class="flex-width-50 base-height ">
					<view class="flex">
						<view class="flex-sub base-height flex align-center">
							<input style="font-size: 26rpx;" :disabled="true" @tap="handleTap('areaName')" v-model="areaName" placeholder="选择所属市" name="areaName"/> 
						</view>
						<view class="flex-width-15 base-height ">
						 <view class="flex-title base-height justify-center align-center">
...
export default {
	...
	methods: {
		...
		// 验证手机短信验证码
		checkPhoneSmsCode(){
			let _url = `${this.$config.wxBaseUrl}/wap/wechat/checkPhoneSmsCode`
			let params = {
		...
		sendCode() {
		  if (!this.rightMobile.status) {
			...
			let _url = `${this.$config.wxBaseUrl}/wap/wechat/sendPhoneSmsCode`
			let params = {
				phoneNo:this.verificationForm.mobile
			}
			this.getOrPostDataBy(_url,params).then(res=>{
		...
		getFeedbackInfoByOpenId(){
			...
			let _url = `${this.$config.wxBaseUrl}/wap/wechat/getFeedbackInfoByOpenId`
			let params = {
				entity: {
				applUserOpenId:this.wxOpenId
				        },
				orderField: "",
				pageNumber: 1,
				pageSize: 10,
			...
			this.getOrPostDataBy(_url,params,config).then(res=>{
				if(res && typeof res == 'object'){
					...
					this.formDataItem.companyname= this.FeedbackHistoryInfo.applComp
					this.formDataItem.phoneno= this.FeedbackHistoryInfo.applUserPhone
					this.formDataItem.uname= this.FeedbackHistoryInfo.applUserName
				}else{
					this.FeedbackHistoryInfo ={}
				}
		...
		ChooseImage() {
			if(!!!this.wxOpenId){
				this.$refs['Message'].error('微信授权失败,无法上传!');
				return
			}
			uni.chooseImage({
				count: 1, //默认9
				...
				success: (res) => {
					...
					let imgSize = tempFilesList[0].size/1000/1024
					if(imgSize>3){
						this.$refs['Message'].warn('上传图片不能大于3M')
						return
					}
					...
					for (var i = 0; i < tempFilePaths.length; i++) {
						...
						uni.uploadFile({
							url: `${this.$config.wxBaseUrl}/wap/wechat/submitFeedbackImg`,
							...
							success: (uploadFileRes) => {
								...
								if(!!resData.success){
										 ...
										 _self.$refs['Message'].success('上传成功')
										 // 上传成功后才显示图片
									 ...
									 _self.$refs['Message'].error('上传失败')
									
								}
							
							},
							fail: (error) => {
								this.$refs['Message'].error('上传失败')
			...
			uni.showModal({
				...
				success: res => {
					if (res.confirm) {
					  ...
					  let imgidsList = _self.formDataItem.imgids.split(',')
					  if(imgidsList && imgidsList.length>0){
						 let delItem = imgidsList.splice(e.currentTarget.dataset.index, 1)
						 if(delItem){
							_self.deleteImgById(delItem.toString()) 
						 }
					  }
					}
				}
		...
		//删除图片
		deleteImgById(imgId){
			let _url = `${this.$config.wxBaseUrl}/wap/wechat/deleteImg`
			let params = {
				imgId:imgId,
			...
			this.getOrPostDataBy(_url,params).then(res=>{
				//debugger
				this.formDataItem.imgids = this.formDataItem.imgids.replace(imgId+',','')
				this.$refs['Message'].success('删除图片成功');
			}).catch(err=>{
				this.$refs['Message'].error('删除图片失败!!'+err);
			})
		},
		// 问题大类选择回调
		// RadioChange(e){
		...
		SubmitData(){
			...
			let _url = `${this.$config.wxBaseUrl}/wap/wechat/submitFeedback`
			let params = this.formDataItem
			params.openId = this.wxOpenId
			params.imgids = params.imgids.replace(/,,/g, ',')
			this.getOrPostDataBy(_url,params).then(res=>{
