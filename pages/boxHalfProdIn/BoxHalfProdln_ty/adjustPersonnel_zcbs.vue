<template>
	<view>
		...
		<view>
			<v-table :row-class-name="rowClassNameFn" :columns="findGoodsColumns" :list="paperOutTableDataItems"
				selection="single" :td-width="150" @on-selection-change='onClick'>
		...
		<view class="cu-modal" :class="modalName=='DialogModal2'?'show':''">
			<view class="cu-dialog">
				...
				<view class="updete grid-flex" v-show="modalTitle=='修改'?'show':''">
					...
					<input type="number" style=" border: 1px solid red; width: 30%;"
						v-model="findGoodsTableDataItems.tt_quotient" />
				</view>
			...
			<view class="cu-dialog">
				...
				<view class="grid-body">
					<view class="flex border-top">
						<view class="flex-sub">
							<selectDropdown :otherHeight='CustomBarHeight' ref="ProdutDrawer"
								url="appGetClassWorkerDroplist" title="编号" placeholdertext="请选择工序大类"
								@closeMain='closeMain'
								v-model='uptatefrom.w_ID'
								:params="{modelCode:'workProc_in',pageSize:100,pageNum:1,name:'',workProcId:teamId}">
							</selectDropdown>
						</view>
						<view class="flex-sub">
							<view class="flex-sub">
								<view class="cu-form-group">
									...
									<input disabled v-model="uptatefrom.w_Name" placeholder="请输入姓名" />
								</view>
							</view>
						</view>
					</view>
...
<script>
	...
	export default {
		...
		methods: {
			closeMain(data){
				this.$refs.ProdutDrawer.formItem.customers = data.type
				this.uptatefrom.w_Name = data.w_Name
				this.uptatefrom.tt_quotient = data.tt_quotient
			...
			getpeopleName() {
				...
				request.post('/warehouse/warehouse/execute/appFindWorkerNameByID', data).then(res => {
					if (res.list.length !== 0) {
						...
						this.toast.hide();
						this.uptatefrom.w_Name = res.list[0].workerName //姓名
						this.uptatefrom.tt_quotient = res.list[0].quotient //工分
					} else {
						this.toast.hide();
