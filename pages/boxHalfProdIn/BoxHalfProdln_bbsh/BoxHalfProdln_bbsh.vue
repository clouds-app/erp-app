						...
						<view class="flex border-top">
							...
							<view class="flex-sub">
								<view class="cu-form-group">
									...
									<input  v-model="boxHalfProdIn.Operator"/>
								</view>
							</view>
						</view>
						<view class="flex border-top">
							<view class="flex-sub">
								<view class="cu-form-group">
									...
									<input  v-model="boxHalfProdIn.Remark" placeholder="请输入备注"/>
								</view>
							</view>
						</view>
					</view>
		...
		<!-- <view class="border-bottom"> -->
			<view  class="grid-body" v-for="(item,index) in fromdata" :key='index'>
				<view class="flex border-top">
					<view class="flex-sub">
						<view class="cu-form-group">
							<view class="title">客户:{{item.ct_Desc}}</view>
						</view>
					</view>
				</view>
				<view class="flex border-top">
				...
				<view class="flex border-top">
					<view class="flex-sub">
						<view class="cu-form-group">
							<view class="title">盒式：{{item.bp_CBoxID}}</view>
						</view>
					</view>
				</view>
				<view class="flex border-top">
			...
			<view class="border-bottom" v-if="false">
...
export default {
	...
	methods: {
		...
		closerest(){
			...
			this.$refs.wplDrawer.clearFromData()//清空工序小类
		...
		getwplArray(){
			...
			request.post(`/warehouse/warehouse/execute/appFindWorkProcPriceItem`,this.machineparams).then(res=>{
				if(res.list.length != 0){
					...
					this.toast.hide();
					this.toast.message('没有对应的工序小类，请重新选择！')
					return;
				}
			}).catch(err=>{
				this.toast.hide();
		...
		SubmitData(){
			...
			let data = {
				...
				hp_wpl_Id:this.formItem.wplId,//工序小类
				...
				Operator:this.boxHalfProdIn.Operator,//操作人
				Remark:this.boxHalfProdIn.Remark,//备注
			}
		...
		restFrom(){
			...
			this.boxHalfProdIn.Operator = '';//不良数
			this.boxHalfProdIn.Remark = '';//备注
			// this.boxHalfProdIn.wpl_WorkPriceUnitName = '';//工序小类显示单位
		...
		async turnOnScanCode(type) {
			...
			if(scandata.resp_code==1000){
				...
				this.boxHalfProdIn.hp_bi_WorkNo = b;
				this.findGoods();
			}else{
		...
		findGoods(){
			...
			let data = {
				...
				ProcID:'',//工序小类
				bc_No:'',//订单号
				workProcNo:this.formItem.wplId,//工序小类
			}
		...
		getmonny(){
			...
			let data = {
				wpl_Id:this.formItem.wplId,//工序小类
