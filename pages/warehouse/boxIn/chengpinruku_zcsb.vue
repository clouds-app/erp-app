<template>
	<view>
		<cu-custom class="titlefrom" bgColor="bg-gradual-blue" :isBack="true">
			<block slot="content">纸箱成品入库</block>
		</cu-custom>
		<!-- <view class="flex border-top"> -->
		<!-- <view class="flex-sub" > -->
		<selectDropdown @onChange="onChangeInToCache" :componentConfig="{tokenKey:'wp.classDrawer'}"
			:defaultValue='boxIn.formItem.bi_Group' :otherHeight='CustomBarHeight' ref="classDrawer"
			url="appFindClassBan" title="班ㅤ别" placeholdertext="ㅤ请选择班别" v-model='boxIn.formItem.bi_GroupCode'
			:params='params'></selectDropdown>
		<selectDropdown @onChange="onChangeInToCache" :componentConfig="{tokenKey:'wp.stationDrawer'}"
			:defaultValue='boxIn.formItem.bi_station' :otherHeight='CustomBarHeight' ref="stationDrawer"
			url="appFindSearchStation" title="仓ㅤ位" placeholdertext="ㅤ请选择仓位" v-model='boxIn.formItem.bi_stationCode'
		...
		<view class="flex border-top">
			<view class="flex-sub">
				<view class="cu-form-group">
					<view class="title">本次套数:</view>
					<input type="number" @blur='boxEvent' placeholder="请输入本次套数" v-model="boxbc" />
				</view>
			</view>
			<view class="flex-sub">
				<view class="cu-form-group">
					<view class="title">配套系数:</view>
					<input v-model="boxIn.formItem.BoxPS" disabled />
				</view>
			</view>
		</view>
		<view class="flex border-top">
		...
		<view class="flex border-top">
			...
			<view class="flex-sub">
				<view class="cu-form-group">
					...
					<input v-model="boxIn.formItem.WorkQty" disabled />
				</view>
			</view>
		</view>
		<view class="cu-form-group border-top">
		...
		<view class="cu-form-group border-top">
			...
			<textarea  v-model="boxIn.formItem.Remark" type="text" style="height: 70rpx;" disabled />
		</view>
		<!--  侧边搜索列表 -->
		...
		<view class="flex solid-bottom padding justify-around chucangbtm">
			...
			<button :disabled="boxIn.dataIsLoadding" @click="boxInAspSaveBoxIn" type="primary" size="mini"
				class="bg-grey radius">保存</button>
		</view>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			boxEvent() {
				...
				this.boxIn.formItem.InQty = this.boxbc * this.boxIn.formItem.BoxPS
			},
			// 获取上传的值
			setDefaultValue() {
				// 班别
