<template>
	<view>
		...
		<view class="flex-sub">
			<selectDropdown :otherHeight='CustomBarHeight' ref="group" url="appFindClassBan" title="班ㅤ别ㅤ"
				placeholdertext="请选择班别" v-model='paperInFormItems.group' :params='{modelCode:"paper_in"}'>
			</selectDropdown>
		</view>
		</view>
		<view class="flex border-top">
		<view class="flex-sub">
			<selectDropdown :allSetonlyOne="false" :otherHeight='CustomBarHeight' ref="Pier" url="appFindSearchStation" title="码ㅤ头ㅤ"
				placeholdertext="请选择码头" v-model='paperInFormItems.Pier' :params='{modelCode:"papercw_In",type:"9"}'>
			</selectDropdown>
		</view>
		</view>
...
<script>
	...
	export default {
		...
		mounted() {
			let group = this.$store.state.paperScanIn_zsbt.group
			this.$refs.group.formItem.customers = group
			if (group !== undefined) {
				this.paperInFormItems.group = group.split("_")[0]
			}
			let Pier = this.$store.state.paperScanIn_zsbt.Pier
			this.$refs.Pier.formItem.customers = Pier
			if (Pier !== undefined) {
				this.paperInFormItems.Pier = Pier.split("_")[0]
			}
		},
		methods: {
			// 工单板号扫码失焦事件
			...
			delData() {
				...
				this.$refs.group.formItem.customers = ''
				this.paperInFormItems.Pier = ''
				this.$refs.Pier.formItem.customers = ''
				this.historInfoList = []
			},
			// 入库效验
			ValidateOut(){
			...
			paprtIn() {
				...
				setTimeout(()=>{
					...
					let data = {
						...
						Groups: this.paperInFormItems.group,
						Pier: this.paperInFormItems.Pier,
					};
					this.toast.loading();
			...
			// 销毁页面保存相关信息
			onUnload() {
				this.$store.commit('setPaperScanIn_zsbt', {
					group: this.$refs.group.formItem.customers,
					Pier: this.$refs.Pier.formItem.customers
				})
			}
		}
	}
