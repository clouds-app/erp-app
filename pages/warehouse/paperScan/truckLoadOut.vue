<template>
	<view>
		...
		<view class="flex border-top">
			<view class="flex-sub">
				<selectDropdown @closeMain='closeMain' :otherHeight='CustomBarHeight' ref="loadNo" url="appFindPaperCar"
					title="装车单号" placeholdertext="请装车单号" v-model='paperInFormItems.loadNo'
					:params='{modelCode:"paper_zcout"}'>
				</selectDropdown>
			</view>
		</view>
		<view class="flex border-top">
		...
		<view class="vtable" style="padding-top: 20rpx;">
			<v-table :redColor="redColor" :row-class-name="rowClassNameFn" :columns="dataColumns" :list="dataTableList"
				selection="single" :td-width="155" :height="tableHeight-30" @on-selection-change="subClickitem">
			</v-table>
		</view>
		<!-- 详情弹出层 -->
		<view class="cu-modal" :class="modalName=='DialogModal1'?'show':''" v-if="orderData === '' ? false : true ">
			<view class="cu-dialog" v-if="orderData === '' ? false : true ">
				...
				<scroll-view scroll-y="true" style="height:600rpx;overflow-y:scroll;">
					<view style="font-size: 13px;" class="tagBox padding-10" v-for="(item,index) in orderData"
						:key=index>
						<view class="grid-title">
							...
							<view v-if="!!item.ZCCreate" @click="delInfo(item,index)"
								class="tag-t bg-red radius cu-tag">清除装车</view>
						</view>
						<view class="grid-body">
							<view class="grid-flex padding-10">
								<view>
		...
		<view class="flex solid-bottom padding justify-around chucangbtm">
			<button @click="tableItemInfo" type="primary" size="mini" class="bg-grey radius">详情</button>
			<button @click="paprtIn" type="primary" size="mini" class="bg-grey radius">出仓</button>
		</view>
		<!-- <stockUpOut></stockUpOut> -->
	</view>
</template>
...
<script>
	...
	import stockUpOut from '@/pages/warehouse/paperScan/stockUpOut'
	export default {
		components: {
			cuCustom,
			vTable,
		...
		methods: {
			...
			closeMain(e) {
				...
				request.post('/warehouse/warehouse/execute/appGetSaveCarList', params).then(res => {
					if (res.list[0].isEor === 0) {
						this.toast.message('该装车单号备货未审核通过')
						return
					}
			...
			getParamsFromScanCode() {
				...
				this.toast.loading();
				request.post('/warehouse/warehouse/execute/appZcWorkBanNo', params)
					.then(res => {
						if (res.list[0] === null) {
							this.toast.hide();
			...
			dataTableListData() {
				let flag = true
				this.dataTableList.forEach((item, index) => {
					if (item.ScanQty !== item.ZCScanQty) {
						flag = false
					}
					if (item.ZCScanQty !== 0) {
						this.zcqQty = false
			...
			paprtIn() {
				this.dataTableListData()
				if(this.dataTableList.length === 0) {
					this.toast.message('该工单版号没有数据')
					return
				}
				if(this.zcqQty) {
					this.zcqQty = true
					this.toast.message('该订单号没有装车')
					return
				}
				let loadNo = this.paperInFormItems.loadNo
				if (!this.dataTableListData()) {
					this.zcqQty = true
					uni.showModal({
						title: '提示',
						content: '检测到装车数不等于备货数，是否继续出仓？',
						success: function(res) {
							if (res.confirm) {
								uni.navigateTo({
									url: `./saveOut?zcdh=${loadNo}`
								});
							} else if (res.cancel) {
