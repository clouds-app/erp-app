<template>
	<view>
		<cu-custom bgColor="bg-gradual-blue" :isBack="true">
				<block slot="content">纸箱销售利润表</block>
		</cu-custom>
				...
				<view class="flex-sub">
					<view class="cu-form-group">
						<view class="fixed-left">
							...
							<button :disabled="isLoaddingData" :style="{color: formItem.searchMode=='0'?'red':''}" class=" margin-left cu-btn round line-brown" @click="searchModeClick('0')" >业务</button>
						</view>
				   </view>
		...
		formatDataListBy(dataList){
			if(dataList && Array.isArray(dataList) && dataList.length>0)
			{
				dataList = dataList.map(item=>{
					if(!!item.TaxInAmt){
						item.TaxInAmt = Number((item.TaxInAmt).toFixed(2)).toLocaleString()
					}
					if(!!item.Profits){
						item.Profits = Number((item.Profits).toFixed(2)).toLocaleString()
					}
					if(!!item.ProfitRate){
						item.ProfitRate = Number(item.ProfitRate).toFixed(1)
					}
					return item
				})
			}
